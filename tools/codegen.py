...
CODEGEN_JSON_URL = (
    'https://raw.githubusercontent.com/rabbitmq/'
...
class Codegen:
    ...
    def _add_comment(
        self, value: str, indent: int = 0, prefix: str = '# '
    ) -> None:
    ...
    def _add_documentation(
        self, label: str, documentation: str, indent: int
    ) -> None:
        ...
        if not len(args):
            ...
            for index, arg in enumerate(args):
                ...
                if index == len(args) - 1:
                    self._add_line(
                        '{}: {} = {}) -> None:'.format(
                            self._arg_name(arg['pyname']), annotation, default
                        ),
                        indent,
                    )
                else:
    ...
    def _arg_default(self, arg: dict, repr_=True) -> str | None:
    ...
    def _build_command_basic_properties(
        self, class_name: str, class_id: int, properties: list
    ) -> None:
        ...
        for arg in properties:
            ...
            if label:
                ...
                self._add_line(
                    ':param {}: Deprecated, must be empty'.format(
                        arg['pyname']
                    ),
                    indent,
                    indent + 4,
        ...
        flag_value -= 1
        self._add_line(
            "'{}': {}}}".format(properties[-1]['pyname'], 1 << flag_value),
            indent + 9,
        )
        self._add_line()
        ...
        for arg in properties:
            self._add_line(
                "_{} = '{}'".format(arg['pyname'], self._arg_type(arg)), indent
            )
        ...
        for arg in properties:
            self._add_line(
                'self.{} = {}'.format(arg['pyname'], arg['pyname']), indent
            )
    ...
    def _build_command_method(
        self, class_name: str, class_id: int, method: dict
    ) -> None:
        ...
        for arg in arguments:
            ...
            if name == 'type' and class_name == 'exchange':
                documentation.append(
                    ...
                    f'"{class_name}_type" to not conflict with the Python type '
                    'keyword.'
                )
                needs_linefeed = True
        ...
        if method_xml and method_xml[0].attrib.get('deprecated'):
            deprecated = True
            documentation.append(
                f'\n.. deprecated:: {self.DEPRECATION_WARNING}'
            )
            needs_linefeed = True
        ...
        for arg in arguments:
            ...
            if name == 'arguments' or arg_type in {
                'common.FieldTable',
                'common.Arguments',
            }:
                default = '{}'
            ...
            if not label and domain and (domain.label or domain.documentation):
            ...
            documentation.append(f':param {name}: {label}')
            if name == 'arguments':
                documentation.append(
                    f':type {name}: :const:`~pamqp.common.Arguments`'
                )
            elif arg_type.startswith('common'):
                documentation.append(
                    f':type {name}: :const:`~pamqp.{arg_type}`'
                )
        ...
        if raises:
            for exc_name in raises:
                documentation.append(
                    f':raises {exc_name}: when an argument fails to validate'
                )

        label = (
            self._label({'class': class_name, 'method': method['name']})
        ...
        for arg in arguments:
            self._add_line(
                "_{} = '{}'".format(arg['pyname'], arg['type']), indent
            )
        if len(arguments):
            ...
            for arg in arguments:
                ...
                default = self._arg_default(arg)
                if (
                    (domain and not domain.nullable and domain.type == 'table')
            ...
            def format_deprecated_value(
                ...
                self._add_line(
                    f'if self.{field_name} is not None and self.{field_name} {operator} {expected_value}:',
                    indent,
                )
                self._add_line(
                    f"raise ValueError('{field_name} must be {error_value}')",
                    indent + 4,
                )
            ...
            if add_validate:
                ...
                self._add_line()
                self._add_line(
                    ':raises ValueError: on validation error', indent
                )
                self._add_line()
                self._add_line('"""', indent)
                for arg in arguments:
                    ...
                    if domain and domain.max_length is not None:
                        self._add_line(
                            ...
                            'len(self.{}) > {}:'.format(
                                arg['pyname'], arg['pyname'], domain.max_length
                            ),
                            indent,
                        )
                        line = (
    ...
    def _build_commands(self):
        ...
        for name in [
            c['name']
            for c in self._amqp_json['classes']
    ...
    def _build_constants(self):
        ...
        def _build_domain_output() -> tuple[list[str], list[str], list[str]]:
            data_types, domains, dom_regex = [], [], []
            for domain in self._domains():
                if domain.name == domain.type:
                    value = f"              '{domain.name}',"
                    comment = '{} {}'.format(
                        '#'.rjust(28 - len(value)), domain.label
                    )
                    data_types.append(f'{value}{comment}')
                else:
                    domains.append(
                        f"           '{domain.name}': '{domain.type}',"
                    )
                    if domain.regex:
                        dom_regex.append(
                            f"           '{domain.name}': re.compile(r'{domain.regex}'),"
                        )
        ...
        self._add_line(f"DEPRECATION_WARNING = '{self.DEPRECATION_WARNING}'")
        self._add_line()
    ...
    def _build_exceptions(self):
        ...
        for error_code in errors.keys():
            error_lines.append(
                f'          {error_code}: AMQP{errors[error_code]},'
            )
    ...
    def _documentation(self, search_path: dict) -> str | None:
        """Find the documentation in the xpath"""

        def strip_whitespace(value: list[lxml.etree.Element]) -> str:
            return ' '.join(
    ...
    @functools.lru_cache
    def _domain(self, value: str) -> Domain | None:
        for domain in self._domains():
    ...
    def _domains(self) -> list[Domain]:
        ...
        for value in self._amqp_json['domains']:
            ...
            if (
                ...
                kwargs['default_value'] = extension[0].attrib.get(
                    'default-value'
                )

            for assertion in (
                self._codegen_xml.xpath(
                    f'//amqp/domain[@name="{value[0]}"]/assert'
                )
                or []
            ):
            ...
            for assertion in (
                self._extensions_xml.xpath(
                    f'//rabbitmq/domain[@name="{value[0]}"]/assert'
                )
                or []
            ):
                if assertion.attrib.get('check') == 'length':
    ...
    def _label(self, search_path: dict) -> str | None:
        ...
        if node and 'label' in node[0].attrib:
            ...
            return f'{node[0].text.strip()[0:1].upper()}{node[0].text.strip()[1:].strip()}'
        if 'field' in search_path:  # Look in domains and extensions
            domain = self._domain(search_path['field'])
            if domain and domain.label:
                return f'{domain.label[0:1].upper()}{domain.label[1:].strip()}'
    ...
    def _lookup_field(self, class_name: str, method: str, field: str) -> dict:
        ...
        result = self._codegen_xml.xpath(
            f'//amqp/class[@name="{class_name}"]/method[@name="{method}"]/'
            f'field[@name="{field}"]'
        )
        if result:
        ...
        result = self._extensions_xml.xpath(
            f'//rabbitmq/class[@name="{class_name}"]/method[@name="{method}"]/'
            f'field[@name="{field}"]'
        )
        if result:
            for attribute in result[0].attrib:
    ...
    def _write_file(self, path: pathlib.Path) -> None:
        if self.YAPF:
            code = yapf_api.FormatCode(
                '\n'.join(self._output_buffer), style_config='pep8'
            )[0]
        else:
