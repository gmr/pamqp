...
def _marshal(frame_type: int, channel_id: int, payload: bytes) -> bytes:
    ...
    return b''.join(
        [
            struct.pack('>BHI', frame_type, channel_id, len(payload)),
            payload,
...
def _marshal_content_body_frame(
    value: body.ContentBody, channel_id: int
) -> bytes:
...
def _marshal_content_header_frame(
    value: header.ContentHeader, channel_id: int
) -> bytes:
...
def _unmarshal_protocol_header_frame(
    data_in: bytes,
) -> header.ProtocolHeader | None:
